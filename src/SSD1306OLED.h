...
void ssd1306_command(uint8_t c)
{
    ...
    I2C_Start(i2c0_dev);
    I2C_SetAddress(i2c0_dev, _i2caddr);
    I2C_Write(i2c0_dev, control);
    I2C_Write(i2c0_dev, c);
    I2C_Stop(i2c0_dev);
}
...
void SSD1306_Display(void)
{
  ...
  for (uint16_t i = 0; i < (SSD1306_LCDWIDTH*SSD1306_LCDHEIGHT / 8); i++) {
      ...
      I2C_SetAddress(SSD1306_STREAM, _i2caddr);
...
void SSD1306_DrawBMP(uint8_t x, uint8_t y, uint8_t *bitmap, uint8_t w, uint8_t h)
{
  for (uint8_t i = 0; i < h; i++) {
    for (uint8_t j = 0; j < w; j++) {
      uint16_t index = i*w/8 + j/8;
      uint16_t bit_mask = 1 << (j%8);
      if ((bitmap[index] & bit_mask) != 0) {
        SSD1306_DrawPixel(x + j, y + i, true);
      } else {
        SSD1306_DrawPixel(x + j, y + i, false);
      }
    }
  }
  /*
  ...
  {    
    ...
    {      
      if( bit_test(bitmap[j/8 + i*w], (j % 8)) != 0 )
